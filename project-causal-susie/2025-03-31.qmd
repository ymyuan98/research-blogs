...
$$
\mathcal{L}_i \left(\left\{\theta_{j}^{(0)}, \Delta_{j}^{(1)}, \Delta_{j}^{(2)} \right\}_{j=1}^{p}, \sigma^2 \bigg| y_i, \mathbf{x}_i \right) :=
...
\boldsymbol{\Delta} = \left(\Delta_{j}^{(1)}, \ldots, \Delta_{j}^{(K_j-1)} \right)_{j=1}^p = \delta \boldsymbol{\gamma}, \\
