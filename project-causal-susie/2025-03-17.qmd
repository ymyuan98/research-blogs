...
After all, to estimate $\left(\Delta_j^{(1)}, \Delta_j^{(2)} \right)$ , we need to estimate $\left(\theta_j^{(0)}, \theta_j^{(1)}, \theta_j^{(2)}\right)$. Then it turns to our second encoding version: one-hot encoding.
...
Then, $\Delta_j^{(1)} = \theta_j^{(1)} - \theta_j^{(0)}$ and $\Delta_j^{(2)} = \theta_j^{(2)} - \theta_j^{(0)}$.
...
($\left(\Delta_j^{(1)}, \Delta_j^{(2)}\right)$ are the differences of points in y-axis! Not the slopes of lines between the two points)
...
\mu_1(\Delta^{(k)}) := \mu_1(\mathbf{x, y, \hat{w}}; \sigma^2, \sigma^2_0)(\Delta^{(k)}) := \frac{\sigma^2_1(\Delta^{(k)})}{s^2(\Delta^{(k)})} \times \hat{\Delta}^{(k)}. 
$$
...
Assume that each layer $l$ captures only one $\Delta_j^{(k)}, \ k=1,2$. Let $\boldsymbol{\Delta} =(\Delta_1^{(1)}, \Delta_1^{(2)}, \ldots, \Delta_p^{(1)}, \Delta_p^{(2)})$. The prior distribution assigned to $\boldsymbol{\Delta}$ is:
